...
class BallBounceController_hermite:
    """
    Encapsulates the ball-bouncing control logic as a steppable controller.
    Mirrors the structure of BallBalanceController for drop-in use in the master loop.
    """

    def __init__(self, odrvs, motors, phi0, shared_hit, t0):
        self.odrvs      = odrvs
        self.motors     = motors
        self.phi0       = phi0
        self.shared_hit = shared_hit
        ...
        self.d_0      = geom.inverse_kinematics([0, 0, 0], p.a, p.b)
        self.d_des    = self.d_0.copy()
        self.q_des    = np.array([0.0, 0.0, 0.0])
        self.q_dot_des = np.array([0.0, 0.0, 0.0])

        self.trajectory     = None   # planning state, updated by get_q_des_ball
        self.last_plan_time = -np.inf
        self.last_t_hit     = -np.inf
        self.p_hit          = None
        self.t_hit          = None
    ...
    def step(self):
        ...
        # --- ball detection ---
        p_hit_shared, t_hit_shared, center, _ = self.shared_hit.get()
        if (p_hit_shared is not None
                and not np.any(np.isnan(np.asarray(p_hit_shared)))
                and t_hit_shared is not None
                and t_hit_shared > t):
            self.p_hit = p_hit_shared
            self.t_hit = t_hit_shared

        # --- replan ---
        if (self.p_hit is not None
                and not np.any(np.isnan(np.asarray(self.p_hit)))
                and self.t_hit is not None
                and self.t_hit > t):
            (self.q_des,
            self.trajectory,
            self.last_plan_time,
            self.last_t_hit) = geom.get_q_des_ball(
                t,
                self.trajectory,
                self.last_plan_time,
                self.q_des,
        ...
        # --- follow trajectory if we have one, else hold position ---
        if self.trajectory is not None:
            vel = traj.evaluate_velocity(self.trajectory, t)
            if np.ndim(vel) >= 1 and np.shape(vel) == (3,):
                self.q_dot_des = vel
            else:
